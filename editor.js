...
function handleKeyDown(event) {
    ...
    if (
        ...
        (prevChar && !/\s/.test(prevChar) && nextChar === '') ||
        (prevChar === '' && nextChar && !/\s/.test(nextChar)) ||
        (prevChar === '' && nextChar === '')
    ) {
        isBetweenWords = true;
    } else {
    ...
    if (lastSuggested?.charAt(0) === nextChar)
        isBetweenWords = false;

    if (event.key === 'Backspace')
        isbackspacePressed = true;

    if (event.key === 'Backspace' && lastSuggested && lastWord && lastWordd) {
